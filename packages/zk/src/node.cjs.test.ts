 ...
 *
 *  1. `require('@cluster/zk')` must NOT crash at import. The main barrel is
 *     browser-safe (no node:fs), so its CJS build has no top-level
 *     `import.meta.url` → `fileURLToPath(undefined)` crash.
 *  2. `require('@cluster/zk/node').loadCircuit` must be a function and resolve
...
describe.skipIf(!built)("CJS require smoke", () => {
  it("require('@cluster/zk') does not throw and exposes the browser barrel", () => {
    const out = runNode(
      "const z=require('@cluster/zk'); console.log(typeof z.CircuitProver, typeof z.verifyDisclosure);",
    );
    expect(out).toBe("function function");
  });

  it("require('@cluster/zk/node').loadCircuit resolves a vendored circuit", () => {
    const out = runNode(
      "const z=require('@cluster/zk/node'); console.log(typeof z.loadCircuit, typeof z.loadCircuit('register').bytecode);",
    );
