 ...
 *
 * PORT NOTE (`@cluster/zk`, Z2 does NO RPC): the demo's `verifyDisclosure`
 ...
 * RPC. `@cluster/zk` performs NO network I/O, so the caller (API/web) resolves
 ...
 * `{ ok, amount }` (the frozen `@cluster/zk` surface); throws
