 ...
 * `@cluster/zk`: the demo imported `EventRef`/`TransferEvent` from a
 * network-bound `chain/events.ts`. `@cluster/zk` (Z2) does NO RPC, so the
 ...
 * of band, off the `@cluster/zk` critical path) and feeds the resolved fields
 ...
 * are named explicitly so `@cluster/zk` stays free of any RPC/event-decoding
