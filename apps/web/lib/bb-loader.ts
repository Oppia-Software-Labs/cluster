 ...
 * `import()` to analyze/rewrite (and so no magic-comment is needed). It is
 * constructed lazily inside the loader callback: `new Function` is forbidden on
 * some edge runtimes (EvalError: code generation from strings), and this module
 * gets pulled into the SSR bundle even though proving is browser-only.
 */
import { setUltraHonkBackendLoader } from "@cluster/zk";
...
/** Point @cluster/zk's prover at the native-ESM bb.js. Idempotent; browser-only. */
export function ensureBrowserProver(): void {
